...
df_homicide %>% 
  ggplot(aes(fct_reorder(city_state, desc(OR)), OR)) + geom_point() + geom_errorbar(aes(ymin = conf.low, ymax = conf.high)) + theme(axis.text.x = element_text(angle = 90)) + ggtitle("estimated adjusted ORs and confidence intervals per city, state") + xlab("city, state") + ylab("adjusted ORs of solved cases, non-white vs white")
